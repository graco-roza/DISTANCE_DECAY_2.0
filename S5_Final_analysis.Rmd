...
params:
  based: occ # occ= occurence | abund = abundance
...
results<-  readxl::read_xlsx('input_between/processed_data.xlsx') %>% 
...
# Organism =============================================================================================================
df <- table(Tax$Organism) %>%
  data.frame() %>%
  set_names(c("Organism", "Counts"))
...
```{r Figure 2, fig.height=14, fig.width=13, eval=FALSE}
...
for (kk in 1:nrow(Tax)) {
  ...
  long_env_tax[[kk]] <- data.frame(
    ...
    y = sapply(eh_range, function(x)
      (1 - exp(
        Tax$Intercept_env[kk]
      )) * exp(-Tax$Slope_env[kk] * x)),
...
```{r,save figure 4, fig.width=14, fig.height=8}
figure_4 <-
...
results <-  readxl::read_xlsx('input_between/processed_data.xlsx') %>% 
  ...
  dplyr::mutate(across(c("Slope_spa"), ~ -.x))
...
T_spa <- Tax    %>% dplyr::select(Slope_spa,Latitude, spa_max, Realm, Body_size, Dispersal_mode, n_sites, gamma_spe) 
F_spa <- Func   %>% dplyr::select(Slope_spa,Latitude, spa_max, Realm, Body_size, Dispersal_mode, n_sites, gamma_trait)
T_env <- Tax    %>% dplyr::select(Slope_env, Latitude,  spa_max, Realm, Body_size, Dispersal_mode, n_var, gamma_spe)
F_env <- Func   %>% dplyr::select(Slope_env, Latitude, spa_max, Realm, Body_size, Dispersal_mode, n_var, gamma_trait)


set.seed(123);M1<- dds.brt(data.frame(T_spa), tc= 5, lr=0.001, bf=0.7, family='laplace', simplify=FALSE)
